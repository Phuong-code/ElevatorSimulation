...
public class FrameView implements Runnable
{
	...
	private void startGraphics()
	{
		...
		jframe.add(new JComponent()
		{
			...
			public void paintComponent(Graphics graphics)
			{
				...
				spacer = (int) (getWidth() - (2 * leftOffset) - numLift * liftDimension.width) / (numLift + 1);
				...
				for (int i = 0; i < numLift; i++) // Draw elevators
				{
					...
					//drawStringToSide("" + elevators.get(i).getInfo(), graphics);
	...
	private void drawBackgroundGrid(Graphics graphics, int height, int width)
	{
		...
		graphics.fillRect(0, 0, width, height - gHeight - levelHeight * (numFloors + 1));
		
		graphics.setColor(sky);
		graphics.fillRect(0, 0, leftOffset, height - gHeight);
		graphics.setColor(sky);
		graphics.fillRect(width - leftOffset, 0, width, height - gHeight);
	...
	private Color getStateColor(int i)
	{
		...
		switch (elevators.get(i).getCurrentState())
		{
			case UP:
				color = Color.RED;
				break;
			...
			case STATIONARY:
				...
				break;
			default:
				throw new IllegalArgumentException("Unexpected value: " + elevators.get(i).getCurrentState());
		}
